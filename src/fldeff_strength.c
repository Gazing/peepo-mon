...
{
    // Peepo: an editor-placed boulder is shoved one tile (networked). The vanilla path
    // enables the push status, which only moves real boulder objects (movement type).
    if (PeepoMapEdit_PlacedFieldMoveTarget(OBJ_EVENT_GFX_PUSHABLE_BOULDER))
    {
