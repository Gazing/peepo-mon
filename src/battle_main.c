...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            u32 fixedOtId = 0;
            u32 abilityNum = 0;
            u16 monSpecies = partyData[monIndex].species;

            // Randomize trainer species per the Trainers setting (Off / All / No Bosses).
            if (PeepoRando_ShouldRandomizeTrainer(trainer))
                monSpecies = PeepoRando_MapSpecies(monSpecies, RANDO_KIND_TRAINER);
            ...
            CreateMon(&party[i], monSpecies, partyData[monIndex].lvl, 0, TRUE, personalityValue, otIdType, fixedOtId);
